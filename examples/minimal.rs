fn main() {
    ...
    let first_name = reactor.signal("Jane".to_string());
    let last_name = reactor.signal("Doe".to_string());
    let age = reactor.signal(45);
    ...
    let full_name = reactor.calc((first_name, last_name), join_with_space);

    // We can also define the calculation as a function
    let welcome = reactor.calc((full_name, age), welcome_message);
