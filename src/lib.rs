...
impl ReactiveContext {
    ...
    pub fn new_calc<T: Send + Sync + PartialEq + 'static, C: CalcQuery<T> + 'static>(
        &mut self,
        calculation_query: C,
...
mod test {
    ...
    fn basic() {
        ...
        let lock1 = reactor.new_calc((button1, button2), &Lock::two_buttons);
        assert!(!reactor.read(lock1).unlocked);

        let button3 = reactor.new_signal(Button::OFF);
        let lock2 = reactor.new_calc((button1, button3), &Lock::two_buttons);
    ...
    fn nested_derive() {
        ...
        let d1 = reactor.new_calc((n1, n2), add); // 1 + 10
        let d2 = reactor.new_calc((n3, n2), add); // 100 + 10

        // This derive uses other derives as inputs
        let d3 = reactor.new_calc((d1, d2), add); // 11 + 110
    ...
    fn many_types() {
        ...
        let baz = reactor.new_calc((foo, bar), |(foo, bar)| Baz(foo.0 + bar.0));
    ...
    fn calculate_pi() {
        ...
        let mut iteration = reactor.new_calc((k_0, iter_0), bailey_borwein_plouffe);
        let mut k = reactor.new_calc((k_0,), increment);
        for _ in 0..100_000_000 {
            iteration = reactor.new_calc((k, iteration), bailey_borwein_plouffe);
            k = reactor.new_calc((k,), increment);
        }
        println!(
            "({:#?}) PI: {:.32}",
            start.elapsed(),
